...
func (m UserModel) GetByEmail(email string) (*User, error) {
	...
	defer cancel()
	err := m.DB.QueryRowContext(ctx, query, email).Scan(&user.ID, &user.CreatedAt, &user.Name, &user.Password.hash, &user.Activated, &user.Version)
	if err != nil {
		switch {
...
func ValidatePasswordPlainText(v *validator.Validator, password string) {
	...
	v.Check(len(password) <= 72, "password", "mush not be more than 72 bytes long")
}
...
func ValidateUser(v *validator.Validator, user *User) {
	...
	if user.Password.hash == nil {
		panic("missing password has hfor user")
	}
